...
#define FEEDING_LEVEL_TIMEOUT	3000
...
//	MPU6050_callback(&mpu);
	float pitch = 0;
	float roll = 0;
	if(current_feeding_level <= 1 && pitch > -45 && pitch < 45 && roll > -45 && roll < 45 && HAL_GetTick() - feeding_timer < FEEDING_TIMEOUT){
		switch(CONTROL_Data.feeding_state){
			case MEASURING_BEFORE_FEEDING:
				...
				if(num_mass % 3 == 0){
					...
					if(t_delta_mass <= 1.1){
						...
						uint8_t t_Tx_Buff[20] = {};
						sprintf((char*)t_Tx_Buff, "Start mas: %ld\n", t_mass);
						HAL_UART_Transmit(&huart2, t_Tx_Buff, strlen((char*)t_Tx_Buff), 500);
			...
			case WAITING_FOOD:
				...
				MOTOR_Set_State(MOTOR_OFF);
				long t_current_des_mass = t_total_feeding_mass - ERROR_MASS;
			...
			case MEASURING_AFTER_FEEDING:
				...
				if(num_mass % 3 == 0){
					...
					if(t_delta_mass <= 1.1){
						long t_mass = (temp_mass[0] + temp_mass[1] + temp_mass[2]) / 3;
						uint8_t t_Tx_Buff[20] = {};
						sprintf((char*)t_Tx_Buff, "Stop mass: %ld\n", t_mass);
						HAL_UART_Transmit(&huart2, t_Tx_Buff, strlen((char*)t_Tx_Buff), 500);
						CONTROL_Data.feeding_state = MEASURING_BEFORE_FEEDING;
						measure_timer -= 1000;
